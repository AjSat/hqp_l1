	...
	for rand_seed in range(0,200):
		...
		for i in range(1, pl):
			...
			for j in range(1, i):
				...
				P_seq[i].add_constraint(A_ineq[j]*x <= b_upper[j] + w[j])
				P_seq[i].add_constraint(w[j].sum <= (cs.DM.ones(1, con_per_level)@sol_seq[j].primals[w[j]]).full())
				P_seq[i].add_constraint(w[j] >= 0)

			P_seq[i].add_constraint(A_ineq[i]*x <= b_upper[i] + w[i])
			P_seq[i].add_constraint(w[i]>=0)
			obj = w[i].sum
			# obj += 1e-6*abs(x)**2
		...
		for i in range(1, pl -1):
			...
			P_dt.add_constraint(lams[(i,i)] <= 1)
			...
			P_dt.add_constraint(w[i].sum <= constraint_acc)

			#Adding the complementarity conditions
			# for j in range(1, i):
			# 	temp = A_ineq[j]*x - b_upper[j] - w[j]
			# 	for k in range(lams[(i, j)].shape[0]):
			# 		P_dt.add_constraint(lams[(i,j)][k]*temp[k] == 0)
		...
		for i in range(1, pl):
			...
			if cs.DM.ones(1, con_per_level)@sol_seq[i].primals[w[i]] <= cs.DM.ones(1, con_per_level)@sol_dt.primals[w[i]] - 1e-1:
				print("Failures!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
				failures += 1
				# break
			if cs.DM.ones(1, con_per_level)@sol_seq[i].primals[w[i]] >= cs.DM.ones(1, con_per_level)@sol_dt.primals[w[i]] + 1e-3:
				print("DT gave a more optimal solution!!!!!!!!!!!!!!!!!!!!!!!!!!")
				break
