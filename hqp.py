...
class hqp:
	...
	def create_variable(self, name, size, weight = 0):

		opti = self.opti
		var0 = opti.parameter(size, 1)
		var_dot = opti.variable(size, 1)
		self.variables0 = cs.vertcat(self.variables, var0)
	...
	def create_constraint(self, name, expression, type, priority = 0, options = {}):
		...
		if priority >= 1:
			...
			self.slack_weights[priority] = cs.vertcat(self.slack_weights, slack_weights)
			self.constraints[priority] = cs.vertcat(self.constraints, expression)
	...
	def configure(self, variables_values):

		#Create functions to compute the constraint expressions
		self.number_priorities = len(self.slacks)
		self.constraint_funs = {}

		for i in range(0, number_priorities + 1):
			self.constraint_funs[i] = cs.Function('cons' + str(i), [self.variables0, self.variables_dot], [cs.jacobian(self.constraints[priority], self.variables_dot)])
